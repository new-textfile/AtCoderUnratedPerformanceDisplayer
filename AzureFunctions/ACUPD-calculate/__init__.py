...
#初期設定
def calc_init() -> None:
    logging.info("[ACUPD-Updater(calculate-calc_init)] now initializing...")
    ...
    outputdict: CalcTempData = {"contest_name": contest_name, "is_ratedcontest": is_ratedcontest, \
        ...
        "user_latest_aperf": {}, "aperflist":[], "ratedplace" : 1, \
...
def calc_useraperf_all(default_aperf: int, startidx: int, endidx: int) -> None:
    ...
    if startidx >= len(contest_result): return

    logging.info("[ACUPD-Updater(calculate-calc_useraperf_all)] <contest: " + contest_name + ", default_aperf: " + str(default_aperf)+ ", idx:" + str(startidx) + " - " + str(endidx) + "> now calculating...")
    ...
    logging.info("[ACUPD-Updater(calculate-calc_useraperf_all)] <contest: " + contest_name + ", default_aperf: " + str(default_aperf)+ ", idx:" + str(startidx) + " - " + str(endidx) + "> calculation finished")
...
def calc_userperf_all(startrank: int, endrank: int) -> None:
    logging.info("[ACUPD-Updater(calculate-calc_userperf_all)] <rank:" + str(startrank) + " - " + str(endrank) + ">" + "now calculating...")
    ...
    if startrank > contest_result[-1]["Place"]: return
    ...
    deq: Deque[int] = deque()
    ratedplace = tempdata["ratedplace"]
    ratedcnt: int = 0
    for useridx in range(len(contest_result)):
        if contest_result[useridx]["Place"] < startrank or endrank <= contest_result[useridx]["Place"]: continue
    ...
    if startrank <= contest_result[-1]["Place"] and contest_result[-1]["Place"] < endrank:
    ...
    tempdata["ratedplace"] = ratedplace
    with open(path.calculate_tempdata, "w") as f:
        json.dump(tempdata, f, indent=4)
    
    logging.info("[ACUPD-Updater(calculate-calc_userperf_all)] <rank:" + str(startrank) + " - " + str(endrank) + ">" + "calculation finished")


#計算したパフォーマンスと, 実際のパフォーマンスの値に齟齬がないかを調べる
def is_matched() -> None:
    logging.info("[ACUPD-Updater(calculate-is_matched)] now checking...")
...
def save_userperfs(default_aperf: int, startidx: int, endidx: int) -> None:
    ...
    logging.info("[ACUPD-Updater(calculate-save_userperfs)] <default_aperf: " + str(default_aperf)+ ", idx: " + str(startidx) + " - " + str(endidx)  + "> now saving...")
    ...
    #全コンテスタントのデータの保存が終わったら一度だけ実行する
    if startidx <= len(contest_result)-1 and len(contest_result)-1 < endidx:
        #tempdataを更新
...
def main(name:str) -> str:
    ...
    if name.split("@")[0] == "init":
        ...
        if name.split("@")[0] == "perf":
            ...
            calc_userperf_all(startidx,endidx)

        elif name.split("@")[0] == "check":
            is_matched()

        else:
            startidx = int(name.split("@")[1].split("-")[0])
            endidx = int(name.split("@")[1].split("-")[1])
            default_aperf = tempdata["pred_default_aperf"] if name.split("@")[2] == "default" else int(name.split("@")[2])

            if name.split("@")[0] == "aperf":
                calc_useraperf_all(default_aperf,startidx,endidx)
