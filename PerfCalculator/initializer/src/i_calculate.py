...
#初期設定
def calc_init(contest_name:str) -> None:
...
def calculate():
    ...
    fetched_contests_info: ContestInfo
    with open(path.new_fetched_contests_info, "r") as f:
        fetched_contests_info = json.load(f)

    fetched_contests.sort(key=lambda x: (datetime.strptime(fetched_contests_info[x]["date"], "%Y-%m-%d %H:%M:%S"), x))

    for contest_name in fetched_contests:
        
        finished: bool = False
        for default_aperf_str in ["default", "800", "1000", "1200", "1600", "2000"]:
            calc_init(contest_name)
